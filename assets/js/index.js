var Index = function () {
   ...
   // function to initiate Revolution Slider
    ...
    // function to initiate Chart 1a
    var runChart1a = function () {
        function randValue() {
        ...
        $("#placeholder-h1a").bind("plothover", function (event, pos, item) {
     ...
     // function to initiate Chart 1b
    var runChart1b = function () {
        function randValue() {
            return (Math.floor(Math.random() * (1 + 40 - 20))) + 20;
        };
        var pageviews = [
        ...
        var previousPoint = null;
        $("#placeholder-h1b").bind("plothover", function (event, pos, item) {
            $("#x").text(pos.x.toFixed(2));
            $("#y").text(pos.y.toFixed(2));
            if (item) {
                if (previousPoint != item.dataIndex) {
    ...
    var runChart3 = function () {
	 ...
	 // Total Sensor Messages Received / Second (main slider)
    var runChart4 = function () {
        ...
        function getRandomData( cb ) {
		    $.ajax({
		      type:'GET',
		      url: 'http://208.96.49.194:3000/api/count/type/all/qty/300',
		      dataType: 'jsonp',
		      success: function(jsondata, status) {
				  $.each(jsondata, function(k, v){
					  data.unshift(v.value);
				  });
	              // Zip the generated y values with the x values
	              var res = [];
	              for (var i = 0; i < data.length; ++i) {
		...
		getRandomData(function( serverdata ){
	        plot = $.plot("#placeholder-h4", [serverdata], {
	            grid: {
	                borderWidth: 1,
	                borderColor: '#eeeeee'
	            },
    ...
    // function to initiate schematic1 (room 1)
    var runschematic1 = function () {
   	   $.ajax({
		    type:'GET',
	    	url: 'http://208.96.49.194:3000/api/sensor/facect_pi0/day/20140522/qty/1',
	      	dataType: 'jsonp',
	      	success: function(data, status) {
	      		$("#schematic1-text").text("Occupants : " + data[0].value);
	      	}, 
	      	error: function(jqXHR, textStatus, errorThrown) {
	      		alert("Error: " + errorThrown); 
	      	}
   	   });
	   setTimeout(runschematic1, 2000);
    };
	// function to initiate schematic1 (room 2)
    var runschematic2 = function () {
   	   $.ajax({
		    type:'GET',
	    	url: 'http://208.96.49.194:3000/api/sensor/facect_pi1/day/20140522/qty/1',
	      	dataType: 'jsonp',
	      	success: function(data, status) {
	      		$("#schematic2-text").text("Occupants : " + data[0].value);
	      	}, 
	      	error: function(jqXHR, textStatus, errorThrown) {
	      		alert("Error: " + errorThrown); 
	      	}
   	   });
	   setTimeout(runschematic2, 2000);
    };
    ...
    return {
        init: function () {
            ...
            runChart1a(); //temp analsyis
            runChart1b(); //humidity analsyis
            //runChart2();
